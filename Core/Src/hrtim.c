...
void MX_HRTIM1_Init(void)
{
  ...
  /* Dead-time ~100ns at 680MHz (DIV1: 1.47ns/tick): 68 ticks */
  HRTIM_DeadTimeCfgTypeDef sDeadTimeCfg = {0};
  sDeadTimeCfg.Prescaler = HRTIM_TIMDEADTIME_PRESCALERRATIO_DIV1;
  sDeadTimeCfg.RisingValue = 68;
  sDeadTimeCfg.FallingValue = 68;
  HAL_HRTIM_DeadTimeConfig(&hhrtim1, HRTIM_TIMERINDEX_TIMER_C, &sDeadTimeCfg);
  HAL_HRTIM_DeadTimeConfig(&hhrtim1, HRTIM_TIMERINDEX_TIMER_D, &sDeadTimeCfg);
  /* UDM (up-down mode) is already set to UP by HAL_HRTIM_WaveformTimerControl
     via TIMxCR2 bit 4 — no manual register write needed */
